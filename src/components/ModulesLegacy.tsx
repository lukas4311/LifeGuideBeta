...
import { progressService } from '@/lib/progress'; // ← Přidej tento import
...
export default function ModulesLegacy() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // 2. Načti VŠECHNY progress data
        const allProgress = await progressService.getAllUserProgress().catch(() => []);
        console.log('📊 All progress:', allProgress); // Debug
  ...
  // ✅ FIX: Správné počítání progressu
  const getModuleProgress = (moduleId: number) => {
    ...
    console.log(`📊 Modul ${moduleId}: ${completedCount}/${module.lessons.length} = ${Math.round((completedCount / module.lessons.length) * 100)}%`);
  ...
  return (
    <div id="modules" className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-12 md:py-20">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-4">{t('modulesTitle')}</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t('moduleSubtitle')}</p>
        </motion.div>
              ...
              return (
                ...
                >
                  <div className="w-full md:w-5/12">
                    <div className={`group relative overflow-hidden rounded-3xl border-2 transition-all duration-300 ${isUnlocked
                      ...
                      <div className="relative h-48 overflow-hidden">
                        <img
                          ...
                          className={`w-full h-full object-cover transition-transform duration-500 ${isUnlocked ? 'group-hover:scale-110' : 'opacity-40'
                            }`}
                        />
                        <div className={`absolute inset-0 bg-gradient-to-t ${module.color} ${isUnlocked ? 'opacity-60' : 'opacity-30'
                          }`} />
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          {!isUnlocked && <Lock className="w-5 h-5 text-gray-400 flex-shrink-0" />}
                        ...
                        {isUnlocked ? (
                          <Link to={createPageUrl('ModuleDetailLegacy') + `?module=${module.id}`}>
                            <Button className={`w-full rounded-xl bg-gradient-to-r ${module.color} text-white hover:shadow-lg transition-all`}>
                  ...
                  <div className="hidden md:flex w-2/12 justify-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center shadow-lg ${isCompleted ? 'bg-green-500' :
                        isUnlocked ? module.bgColor : 'bg-gray-200'
                      }`}>
                      {isCompleted ? (
                        <CheckCircle2 className="w-8 h-8 text-white" />
                      ) : (
                        <span className={`text-2xl font-bold ${isUnlocked ? module.textColor : 'text-gray-400'
                          }`}>
                          {module.id}
                        </span>
                      )}
